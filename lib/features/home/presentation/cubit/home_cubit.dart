...
class HomeCubit extends Cubit<HomeState> {
  ...
  getRandomHomeSurah() async {
    ...
    randomSurah.fold(
      (surah) async {
        ...
        String verseKey = surah.verse.entries
            .firstWhere((entry) => entry.value == randomVerseText)
            .key;
        String verseNumber = verseKey.substring(6);

        print('---->! $randomVerseText');

        // Cache the new Data
        await getIT<CacheHelper>().saveData(
            key: 'randomSurah',
            value: json.encode((surah as SurahModel).toJson()));
        await getIT<CacheHelper>()
            .saveData(key: 'randomVerseText', value: randomVerseText);
        await getIT<CacheHelper>()
            .saveData(key: 'verseNumber', value: verseNumber);
        await getIT<CacheHelper>().saveData(
            key: 'lastGeneratedTime',
      ...
      (failure) {
        ...
        emit(HomeGetFailure(errMessage: failure.errorMessage));
      },
    );
  }
  ...
  Future<void> loadOrGenerateRandomVerse() async {
    final lastGeneratedTime =
        getIT<CacheHelper>().getData(key: 'lastGeneratedTime') ?? 0;
    ...
    if (currentTime - lastGeneratedTime > 24 * 60 * 60 * 1000) {
      ...
      final randomVerseText =
          getIT<CacheHelper>().getData(key: 'randomVerseText');
      final verseNumber = getIT<CacheHelper>().getData(key: 'verseNumber');

      if (randomSurahJson != null &&
          randomVerseText != null &&
          verseNumber != null) {
